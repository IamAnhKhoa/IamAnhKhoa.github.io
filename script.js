...
document.addEventListener('DOMContentLoaded', () => {
    console.log("Applying feature enhancements v16 (Time-based Auto Theme)...");
    ...
    if (oldThemeToggle && header) {
        ...
        // MỚI: Gắn lại sự kiện cho nút theme, có lưu lựa chọn thủ công
        document.getElementById('themeToggle').addEventListener('click', () => {
            const isDark = document.body.classList.toggle('dark');
            localStorage.setItem('theme', isDark ? 'dark' : 'light'); // Lưu lựa chọn
        });
    }
    ...
    // MỚI: Thêm hàm áp dụng giao diện tự động
    applyAutoTheme();
...
function applyAutoTheme() {
    // Chỉ áp dụng khi người dùng chưa tự chọn theme
    if (localStorage.getItem('theme')) {
        return;
    }
    const currentHour = new Date().getHours();
    // Chế độ sáng từ 6h đến 17h59
