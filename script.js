...
const notifications = [
    {
        ...
        content: 'Bộ lọc "Không Khám (chỉ có Thuốc/DVKT)", Không thuốc, hoặc chỉ có DVKT'
    },
    {
        id: 5,
        date: '2025-08-13',
...
document.addEventListener('DOMContentLoaded', () => {
    console.log("Applying feature enhancements v21 (CRITICAL FIX for infinite loop)...");
    ...
    // ===================================================================
    // BƯỚC 3: MỞ RỘNG LOGIC GỐC MÀ KHÔNG SỬA FILE
    // ===================================================================
    ...
    // SỬA LỖI: Viết lại hàm bọc updateDashboard để loại bỏ MutationObserver, khắc phục lỗi treo
    if (typeof updateDashboard === 'function') {
        const original_updateDashboard = updateDashboard;
        updateDashboard = function() {
            // 1. Chạy hàm gốc để nó tính toán và điền các giá trị ban đầu
            original_updateDashboard();

            // 2. Sau khi hàm gốc chạy xong, thực hiện logic nâng cao của chúng ta
            if (globalData.allRecords.length > 0) {
                // Lấy các giá trị gốc mà script vừa điền vào
                const rawTotalAmount = parseFloat(document.getElementById('totalAmount').textContent.replace(/[^0-9]/g, ''));
                const rawTotalBncct = parseFloat(document.getElementById('totalBncct').textContent.replace(/[^0-9]/g, ''));
                
                // Tính giá trị cho thẻ mới
                const totalNguonKhac = globalData.allRecords.reduce((sum, record) => sum + (record.t_nguonkhac || 0), 0);

                // Áp dụng định dạng viết tắt và tooltip cho TẤT CẢ các thẻ
                updateStatCard('totalAmount', rawTotalAmount);
                updateStatCard('totalBncct', rawTotalBncct);
                updateStatCard('totalNguonKhacDashboard', totalNguonKhac);
                
                // Cập nhật mục "Trường hợp đặc biệt"
...
function renderSpecialCases() { const filterValue = document.getElementById('specialCaseFilter').value; const resultsDiv = document.getElementById('specialCaseResults'); if (!filterValue) { resultsDiv.innerHTML = '<p class="case-placeholder">Vui lòng chọn một loại để xem danh sách.</p>'; return; } let filteredRecords = []; switch (filterValue) { case 'no_kham': filteredRecords = globalData.allRecords.filter(r => (r.t_thuoc > 0 || r.t_dvkt_khac > 0) && !r.services.some(s => (s.ten_dich_vu || '').toLowerCase().includes('khám'))); break; case 'no_thuoc': filteredRecords = globalData.allRecords.filter(r => r.t_thuoc === 0 && (r.t_kham > 0 || r.t_dvkt_khac > 0)); break; case 'only_dvkt': filteredRecords = globalData.allRecords.filter(r => r.t_dvkt_khac > 0 && r.t_kham === 0 && r.t_thuoc === 0); break; } if (filteredRecords.length === 0) { resultsDiv.innerHTML = `<p class="case-placeholder">Không tìm thấy hồ sơ nào phù hợp.</p>`; } else { let listHTML = `<ul>`; filteredRecords.forEach(r => { listHTML += `<li><span class="case-info">${r.hoTen} (${r.maBn || r.maLk})</span><span class="case-date">Vào: ${formatDateTimeForDisplay(r.ngayVao)}</span></li>`; }); listHTML += `</ul>`; resultsDiv.innerHTML = `<p><strong>Tìm thấy ${filteredRecords.length} hồ sơ:</strong></p>${listHTML}`; } }
...
function updateStatCard(elementId, fullValue) { const el = document.getElementById(elementId); if (el) { el.textContent = formatCurrencyAbbreviated(fullValue); el.title = `Chính xác: ${formatCurrencyWithDecimals(fullValue)}`; } }
