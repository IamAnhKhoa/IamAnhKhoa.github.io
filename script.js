...
document.addEventListener('DOMContentLoaded', () => {
    console.log("Applying feature enhancements v28 (Fix Click-to-View Details)...");
    ...
    const newStyles = `
        ...
        .details-popup { position: absolute; display: none; background-color: #2c3e50; color: white; padding: 10px 15px; border-radius: 8px; box-shadow: 0 5px 15px rgba(0,0,0,0.2); z-index: 1200; font-size: 1.1em; font-weight: 600; pointer-events: none; animation: fadeInScale 0.2s ease-out; } .details-popup::after { content: ''; position: absolute; bottom: -10px; left: 50%; transform: translateX(-50%); border-width: 5px; border-style: solid; border-color: #2c3e50 transparent transparent transparent; } body.dark .details-popup { background-color: #f8f9fa; color: #2c3e50; } body.dark .details-popup::after { border-color: #f8f9fa transparent transparent transparent; }
    `;
    ...
    initializeSpecialCases(); document.addEventListener('click', hideDetailsPopup);
...
function updateStatCard(elementId, fullValue) { const el = document.getElementById(elementId); if (el) { const fullValueFormatted = formatCurrencyWithDecimals(fullValue); el.textContent = formatCurrencyAbbreviated(fullValue); el.setAttribute('data-full-value', fullValueFormatted); el.onclick = showDetailsPopup; } }
function showDetailsPopup(event) { event.stopPropagation(); const target = event.currentTarget; const popup = document.getElementById('detailsPopup'); const fullValueText = target.getAttribute('data-full-value'); if (popup && fullValueText) { popup.innerHTML = `Chính xác: ${fullValueText}`; const targetRect = target.getBoundingClientRect(); popup.style.display = 'block'; const popupRect = popup.getBoundingClientRect(); popup.style.left = `${targetRect.left + (targetRect.width / 2) - (popupRect.width / 2)}px`; popup.style.top = `${targetRect.top - popupRect.height - 10}px`; } }
function hideDetailsPopup(event) { const popup = document.getElementById('detailsPopup'); if (popup && !popup.contains(event.target) && !event.target.closest('.stat-card h3')) { popup.style.display = 'none'; } }
