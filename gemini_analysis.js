 ...
 * =============================
 * Version 4 (Bugfix):
 * - SỬA LỖI NGHIÊM TRỌNG: Khắc phục lỗi "originalHoSoData is not defined" bằng cách truyền dữ liệu chính xác đến hàm phân tích AI.
 *
...
document.addEventListener('DOMContentLoaded', () => {
    console.log("Gemini Analysis feature v4 (Bugfix) loaded.");

    // ===================================================================
    // BƯỚC 1: TIÊM CSS CHO CÁC THÀNH PHẦN MỚI
    // ===================================================================
    const aiStyles = `
        /* --- Dropdown cho Tab "Kiểm tra XML" --- */
        ...
        .searchable-select-options div:hover, .searchable-select-options div.selected { background-color: #f0f0f0; }
        body.dark .searchable-select-options { background: #1f2937; border-color: #374151; }
        body.dark .searchable-select-options div:hover, body.dark .searchable-select-options div.selected { background-color: #374151; }
    ...
    openAiModalButton.addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); populatePatientSelector(); aiModal.classList.add('show'); });
    ...
    function populatePatientSelector() {
        ...
        const optionsHTML = globalData.allRecords.map((record, index) => `<div data-value="${record.maLk}" data-text="${index + 1}. ${record.hoTen} (Mã LK: ${record.maLk})">${index + 1}. ${record.hoTen} (Mã LK: ${record.maLk})</div>`).join('');
    ...
    patientSearchInput.addEventListener('input', () => { const filter = patientSearchInput.value.toLowerCase(); patientOptionsContainer.querySelectorAll('div').forEach(option => { const text = option.textContent.toLowerCase(); option.style.display = text.includes(filter) ? '' : 'none'; }); patientOptionsContainer.classList.add('show'); patientSelectorValue.value = ''; validateInputs(); });
    ...
    function createPdfHtmlContent(analysisTextFromGemini, originalPatientInfo) {
        return `
            <html>
            <head>
                <meta charset="UTF-8">
                <title>Phan Tich AI - ${escapeBasicHtml(originalPatientInfo.maLk) || 'HoSo'}</title>
                <style>
                    body { font-family: 'DejaVu Sans', Arial, sans-serif; line-height: 1.5; margin: 20px; font-size: 11px; }
                    h1 { color: #2c3e50; text-align: center; border-bottom: 1px solid #3498db; padding-bottom: 8px; font-size: 1.5em; margin-bottom: 15px;}
                    h2 { color: #3498db; margin-top: 20px; border-bottom: 1px solid #bdc3c7; padding-bottom: 4px; font-size: 1.2em;}
                    p { margin-bottom: 8px; text-align: justify;}
                    hr { border: 0; height: 1px; background: #ccc; margin: 20px 0; }
                    ul { margin-left: 20px; padding-left: 0;}
                    li { margin-bottom: 5px; }
                    pre { background-color: #f0f0f0; padding: 8px; border: 1px solid #ddd; border-radius: 4px; white-space: pre-wrap; word-wrap: break-word; font-size: 0.85em; }
    ...
    async function getGeminiAnalysis(promptData, patientInfoForPrompt, generalInfo, originalHoSoData, apiKey) {
        ...
        // NÂNG CẤP PROMPT TOÀN DIỆN
        const fullPrompt = `Bạn là một bác sĩ đa khoa danh tiếng, dược sĩ nhiều năm kinh nghiệm, và chuyên gia giám định BHYT Việt Nam.
        Dựa trên dữ liệu XML tóm tắt được cung cấp, hãy tạo một báo cáo HTML chi tiết theo mẫu sau.
        ...
        **YÊU CẦU ĐỊNH DẠNG HTML (TUÂN THỦ NGHIÊM NGẶT):**
        ...
        <h3>Dữ liệu XML tóm tắt đã cung cấp cho AI:</h3>
